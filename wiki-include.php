...
class Wiki {
   ...
   /*
    * Receive an array and return it as an object
    * */
    
     //array to object creation
	...
	switch ($_REQUEST['action']) {
		...
		foreach($children as $utchild){
			$newchildren[] = $utchild;
		}
		
		$create_newarrray = array(
			'ID' => -10,
			'img' => $img = plugins_url('/',__FILE__).'extra-images/subwiki_addnew.jpg',
			'plink' => get_permalink($post->ID).'?action=edit&eaction=create',
			'title' => 'Create New'
		);
		
		$newchildren[] = (object)$this->arrayToObject($create_newarrray);
		
		
		
		
		$j = 1;
		for($i=0; $i<=count($newchildren);$i++){
			$newcrumbs[$j][] = $newchildren[$i];
			if($i == 0 ) continue;
			if(fmod($i,4) == 0){
				$j++;
			}
		...
		$frame_link = plugins_url('/',__FILE__).'extra-images/subwiki-overlay.png';
		
		$tr = '';
		foreach($newcrumbs as $crums){
			
			$tr .= '<tr>';
			$td = '';
			foreach($crums as $crum){
				
				//checking if it create new button
				if(empty($crum)) continue;
				
				if( $crum->ID == -10){
					
					$permalink = $crum->plink;
					$attachment = $crum->img;
					$title = $crum->title;
					
				}
				else{
					$permalink = get_permalink($crum->ID);
					$title = $crum->post_title;				
								
					//here is the new function to solve the problem
					
					
					$attachment = wp_get_attachment_thumb_url(get_post_meta($crum->ID,'_thumbnail_id', true));
				}			
								
				
				if(strlen($attachment)<7){
					$attachment = plugins_url('/',__FILE__).'extra-images/subwiki_default.jpg';
				}
			
				$td .= "<td><div class='wiki-subwiki-thumbnail-set'>
				 <a href='$permalink'><img style='background:url($attachment)' src='$frame_link' /></a>
				 
				 
				 <p><a href='$permalink'><span class='subwiki-title'> $title </span></a></p>
		...
		$bottom = '';
		if(count($children)>0) {		
			$bottom = "<h3>".__('Sub Wikis', $this->translation_domain) . "</h3><table class='wib-wiki-thumbnail-table' >$tr</table>";
		}
	...
	$supports = array( 'title', 'editor', 'author','thumbnail', 'revisions', 'comments', 'page-attributes');
